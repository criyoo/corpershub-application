...
def _normalize_nin_gender(value: str | None) -> str:
...
def _normalize_nin_birth_date(value) -> str:
...
def _birth_date_components_match(profile_value, nin_value) -> bool:
    normalized_profile = _normalize_nin_birth_date(profile_value)
    normalized_nin = _normalize_nin_birth_date(nin_value)
    if not normalized_profile or not normalized_nin:
        return False

    profile_date = parse_date(normalized_profile[:10])
    nin_date = parse_date(normalized_nin[:10])
    if profile_date is not None and nin_date is not None:
        return (
            profile_date.year == nin_date.year
            and profile_date.month == nin_date.month
            and profile_date.day == nin_date.day
        )

    return normalized_profile == normalized_nin


def _validate_corper_nin_payload(
    ...
    *,
    ...
    if (
        corper.middle_name
        and data.get("middleName")
        and normalize_person_name(corper.middle_name) != normalize_person_name(data.get("middleName"))
    ):
        mismatches.append("Middle name does not match the NIN record.")
    if normalize_person_name(corper.surname) != normalize_person_name(data.get("surname")):
        mismatches.append("Surname does not match the NIN record.")
    if not nin_phone_numbers_match(corper.mobile_number, data.get("telephoneNo")):
        mismatches.append("Mobile number does not match the NIN record.")
    if _normalize_nin_gender(corper.gender) != _normalize_nin_gender(data.get("gender")):
        mismatches.append("Gender does not match the NIN record.")
    if not _birth_date_components_match(corper.date_of_birth, data.get("birthDate")):
        mismatches.append("Date of birth does not match the NIN record.")
    if _normalize_nin_value(submitted_nin) != returned_nin:
        mismatches.append("NIN does not match the NIN record.")
    if (
        corper.state_of_origin
        and data.get("selfOriginState")
        and normalize_free_text(corper.state_of_origin) != normalize_free_text(data.get("selfOriginState"))
    ):
        mismatches.append("State of origin does not match the NIN record.")
    if (
        corper.country_of_birth
        and data.get("birthCountry")
        and normalize_country_text(corper.country_of_birth) != normalize_country_text(data.get("birthCountry"))
    ):
        mismatches.append("Country of birth does not match the NIN record.")
    return mismatches
    ...
    def create(self, request, *args, **kwargs):
        ...
        if verification_type == VerificationAttempt.VerificationType.NIN:
            ...
            for field_name, field_value in (
                ("first name", corper.first_name),
                ("surname", corper.surname),
                ("date of birth", corper.date_of_birth),
                ("gender", corper.gender),
                ("mobile number", corper.mobile_number),
            ):
                if not field_value:
                    missing_profile_fields.append(field_name)
            if missing_profile_fields:
                error_message = (
                    "Submit your biodata first so corpershub can verify your NIN against "
            ...
            if corper.nin_lookup_hash == submitted_hash:
        ...
        if verification_type == VerificationAttempt.VerificationType.CALLUP:
            submitted_hash = hash_sensitive_identifier(submitted_value)
            if corper.nysc_callup_lookup_hash == submitted_hash:
        ...
        if verification_type == VerificationAttempt.VerificationType.STATE_CODE:
            submitted_hash = hash_sensitive_identifier(submitted_value)
            if corper.nysc_state_code_lookup_hash == submitted_hash:
            ...
            with transaction.atomic():
                if verification_type == VerificationAttempt.VerificationType.BIODATA:
                    ...
                    if not payload.get("status") or not data:
                        ...
                        mismatches = _validate_corper_nin_payload(
                            corper,
                            data,
                            submitted_nin=submitted_value,
                        )
                        if mismatches:
                            review_note = " ".join(mismatches)
                            attempt = VerificationAttempt.objects.create(
                                corper=corper,
                                verification_type=verification_type,
                                status=VerificationAttempt.Status.REJECTED,
                            ...
                            latest_biodata_attempt = _get_latest_biodata_attempt(corper)
                            if latest_biodata_attempt is None:
                                latest_biodata_attempt = VerificationAttempt.objects.create(
                                    corper=corper,
                                    verification_type=VerificationAttempt.VerificationType.BIODATA,
                                    status=VerificationAttempt.Status.APPROVED,
                                    submitted_value_masked=_mask_biodata_submission(
                                        corper.full_name,
                                        corper.date_of_birth,
                                        corper.university_matriculation_number,
                                    ),
                                    metadata={
                                        "submitted": True,
                                        "full_name": corper.full_name,
                                        "first_name": corper.first_name,
                                        "middle_name": corper.middle_name,
                                        "surname": corper.surname,
                                        "date_of_birth": str(corper.date_of_birth or ""),
                                        "gender": corper.gender,
                                        "mobile_number": corper.mobile_number,
                                        "state_of_origin": corper.state_of_origin,
                                        "country_of_birth": corper.country_of_birth,
