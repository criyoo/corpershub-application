...
class Conversation(UUIDPrimaryKeyModel):
    ...
    corper = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="corper_conversations"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["company", "corper"], name="unique_conversation_participants")
        ]
...
class Message(UUIDPrimaryKeyModel):
    ...
    delivery_state = models.CharField(
        max_length=20, choices=DeliveryState.choices, default=DeliveryState.SENT
    )
