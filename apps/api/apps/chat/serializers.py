...
class CommunityTopicSerializer(serializers.Serializer):
    ...
    def to_representation(self, instance):
        data = {
            ...
            "message_count": getattr(instance, "message_count", 0) or 0,
            "unread_count": getattr(instance, "unread_count", 0) or 0,
        }
...
class CommunityMessageSerializer(serializers.Serializer):
    id = serializers.UUIDField(read_only=True)
    sender = serializers.UUIDField(source="sender.id", read_only=True)
    sender_role = serializers.CharField(read_only=True)
    sender_name = serializers.CharField(allow_null=True, read_only=True)
    sender_profile_photo = serializers.CharField(allow_null=True, read_only=True)
    content = serializers.CharField()
    created_at = serializers.DateTimeField(read_only=True)

    def to_representation(self, instance):
        data = super().to_representation(instance)
        # sender already serialized above as sender.id
        # Get profile photo URL from corper_profile if available
        if hasattr(instance.sender, "corper_profile"):
            profile = instance.sender.corper_profile
            if profile.profile_photo:
                data["sender_profile_photo"] = get_file_url(profile.profile_photo)
            data["sender_name"] = profile.full_name or "Corper"
        else:
            data["sender_profile_photo"] = None
            data["sender_name"] = "Corper"
        return data
...
class CommunityThreadSerializer(serializers.Serializer):
    topic = CommunityTopicSerializer()
    messages = CommunityMessageSerializer(many=True)
