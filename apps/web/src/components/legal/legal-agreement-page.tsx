...
function hasPendingCorperVerificationDraft(draft: ReturnType<typeof useCorperVerificationDraft>["draft"]) {
  const fullName = [
    draft.biodata.first_name,
    draft.biodata.middle_name,
    draft.biodata.surname,
  ]
...
export function LegalAgreementPage({
  ...
  async function submitTerms() {
    ...
    try {
      ...
      if (session) {
        updateSession({
          ...
          user: {
            ...
            profile_path: config.successRedirect,
      ...
      router.replace(config.successRedirect);
    } catch (error) {
      if (role === "corper") {
    ...
    <DashboardShell role={role} title={config.title}>
      {!profile.data && profile.loading ? (
        ...
        <Card className="grid gap-4">
          ...
          <div>
            <Button type="button" onClick={() => router.push(config.successRedirect)}>
              Continue
            </Button>
          </div>
        </Card>
