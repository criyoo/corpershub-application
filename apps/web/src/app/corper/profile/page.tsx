...
const GRADUATION_YEAR_OPTIONS = Array.from({ length: 2050 - 2000 + 1 }, (_, index) => String(2000 + index));
...
function mapProfileToForm(profile: CorperProfile): FormValues {
  return {
    ...
    bio: profile.bio ?? "",
    preferred_sector: profile.preferred_sector ? profile.preferred_sector.split(",").map((s) => s.trim()).filter(Boolean) : [],
    preferred_organization_type: profile.preferred_organization_type ? profile.preferred_organization_type.split(",").map((s) => s.trim()).filter(Boolean) : [],
...
function CorperProfilePageContent() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (canEditFullProfile) {
      ...
      formData.set("preferred_sector", formValues.preferred_sector.join(", "));
      formData.set("preferred_organization_type", formValues.preferred_organization_type.join(", "));
