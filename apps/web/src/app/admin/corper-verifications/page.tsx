          ...
          corpers.data?.results.map((corper) => [
            ...
            <div key={`${corper.id}-actions`} className="grid gap-3">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-xs uppercase tracking-[0.18em] text-mist">NIN</span>
                <Button
                  type="button"
                  className="px-4 py-2 text-xs"
                  disabled={activeActionId === `${corper.id}-nin_verification_status-verified`}
                  onClick={() => void updateStatus(corper.id, "nin_verification_status", "verified")}
                >
                  Approve
                </Button>
                <Button
                  ...
                  variant="danger"
                  className="px-4 py-2 text-xs"
                  disabled={activeActionId === `${corper.id}-nin_verification_status-rejected`}
                  onClick={() => void updateStatus(corper.id, "nin_verification_status", "rejected")}
                >
                  Reject
                </Button>
              </div>
