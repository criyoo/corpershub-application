...
API_DIR="$APPLICATION_ROOT/apps/api"
WEB_DIR="$APPLICATION_ROOT/apps/web"
...
set_environment_defaults() {
  ...
  if [[ "$ENVIRONMENT" == "prod" ]]; then
    WEB_DOMAIN="corpershub.ng"
    API_DOMAIN="api.corpershub.ng"
  else
    WEB_DOMAIN="$ENVIRONMENT.corpershub.ng"
    API_DOMAIN="api.$ENVIRONMENT.corpershub.ng"
  fi

  AWS_WORKLOAD_PROFILE="$ENVIRONMENT-$PROJECT_NAME"
  NAME_PREFIX="$PROJECT_NAME-$ENVIRONMENT"

  export WEB_DOMAIN API_DOMAIN AWS_REGION AWS_WORKLOAD_PROFILE NAME_PREFIX
}
