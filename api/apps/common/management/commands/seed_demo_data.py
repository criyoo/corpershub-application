...
DEFAULT_COMPANY_SEED_PATH = "seed_demo_data/companies/default_companies.json"
DEFAULT_CORPER_SEED_PATH = "seed_demo_data/corpers/default_corpers.json"
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if not getattr(settings, "SEED_DEMO_ACCOUNTS", False):
            self.stdout.write("SEED_DEMO_ACCOUNTS is false; skipping default account seed.")
            return
    ...
    def resolve_candidates(self, raw_path: str) -> list[Path]:
        ...
        if raw.is_absolute():
            ...
            if "seed_demo_data/" in raw_text:
                ...
                candidates.append(base_dir / "seed_demo_data" / suffix)
