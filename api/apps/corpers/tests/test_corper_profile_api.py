...
class CorperProfileAPITests(APITestCase):
    ...
    def test_verified_profile_allows_posting_state_skill_and_bio_updates(self):
        ...
        self.assertEqual(corper.approval_status, CorperProfile.ApprovalStatus.APPROVED)
        self.assertEqual(response.data["approval_status"], CorperProfile.ApprovalStatus.APPROVED)
        self.assertEqual(
    ...
    def test_biodata_verification_submission_stays_pending_until_review(self):
        ...
        self.assertEqual(attempt.metadata["full_name"], "Ada Grace Lovelace")
        corper = ensure_corper_profile(self.corper_user)
        self.assertEqual(corper.biodata_verification_status, CorperProfile.SensitiveStatus.PENDING)
        self.assertEqual(corper.full_name, "Ada Grace Lovelace")
    ...
    def test_nysc_document_submission_stores_uploaded_files_and_attempt_metadata(self):
        ...
        self.assertEqual(callup_attempt.status, VerificationAttempt.Status.PENDING)
        self.assertEqual(state_code_attempt.status, VerificationAttempt.Status.PENDING)
    ...
    def test_nin_verification_can_be_submitted_without_biodata(self, dikript_lookup_mock):
        response = self.client.post(
            "/api/verification/attempts/",
            {
                "verification_type": VerificationAttempt.VerificationType.NIN,
        ...
        self.assertEqual(attempt.status, VerificationAttempt.Status.APPROVED)
        dikript_lookup_mock.assert_called_once()
