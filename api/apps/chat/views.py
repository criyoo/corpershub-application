...
class CommunityThreadsListAPIView(generics.ListAPIView):
    serializer_class = CommunityTopicSerializer
    pagination_class = None

    def get_queryset(self):
        return get_user_community_topics(user=self.request.user)


class CommunityThreadAPIView(APIView):
    def get(self, request, *args, **kwargs):
        if request.user.role != "corper":
            return Response({"detail": "Only corpers can access community chat."}, status=403)
        topic_id = self.kwargs["topic"]
        thread = get_community_thread(topic_id, user=request.user)
        if thread is None:
            return Response({"detail": "Topic not found."}, status=404)
        mark_community_messages_read(topic_id=topic_id, user=request.user)
        return Response(CommunityThreadSerializer(thread).data)


class CommunityThreadMessagesAPIView(generics.ListCreateAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if thread is None:
            return Response({"detail": "Topic not found."}, status=404)
        return Response(CommunityMessageSerializer(thread["messages"], many=True).data)
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer.is_valid(raise_exception=True)
        message = create_community_message(
            topic_id=topic_id,
            sender=request.user,
            content=serializer.validated_data["content"],
        )
        broadcast_community_message(message)
        return Response(
            CommunityMessageSerializer(message).data,
            status=status.HTTP_201_CREATED,
        )
