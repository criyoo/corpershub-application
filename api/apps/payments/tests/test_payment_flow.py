...
class PaymentFlowTests(APITestCase):
    ...
    @override_settings(FLUTTERWAVE_WEBHOOK_URL="https://local-tunnel.trycloudflare.com/api/payments/webhooks/flutterwave/")
    @patch.object(
        FlutterwaveGateway,
        "initialize_payment",
        return_value=build_flutterwave_checkout("CC-CLOUDFLAREWEBHOOK123"),
    )
    def test_initiate_payment_uses_configured_flutterwave_webhook_url(self, mock_initialize):
        self.client.force_authenticate(user=self.corper_user)
        ...
        self.assertEqual(response.status_code, 201)
        self.assertEqual(
            mock_initialize.call_args.kwargs["webhook_url"],
            "https://local-tunnel.trycloudflare.com/api/payments/webhooks/flutterwave/",
        )

    @patch.object(
        FlutterwaveGateway,
        "initialize_payment",
        return_value=build_flutterwave_checkout("CC-CLOUDFLAREFILE123"),
    )
    def test_initiate_payment_uses_cloudflare_tunnel_file_for_webhook_url(self, mock_initialize):
        self.client.force_authenticate(user=self.corper_user)

        with tempfile.TemporaryDirectory() as tmpdir:
            tunnel_file = Path(tmpdir) / "tunnel-url"
            tunnel_file.write_text("https://file-tunnel.trycloudflare.com\n", encoding="utf-8")
            with override_settings(
                FLUTTERWAVE_WEBHOOK_URL="",
                CLOUDFLARE_TUNNEL_URL_FILE=str(tunnel_file),
                FLUTTERWAVE_WEBHOOK_PATH="/api/payments/webhooks/flutterwave/",
            ):
                response = self.client.post(
                    "/api/payments/transactions/initiate/",
                    {
        ...
        self.assertEqual(
            mock_initialize.call_args.kwargs["webhook_url"],
            "https://file-tunnel.trycloudflare.com/api/payments/webhooks/flutterwave/",
        )
