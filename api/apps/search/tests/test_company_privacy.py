...
class CompanyPrivacySearchTests(APITestCase):
    ...
    def test_corper_on_free_trial_cannot_search_companies_by_sector(self):
        self.client.force_authenticate(self.corper_user)

        response = self.client.get("/api/search/companies/", {"sector": "Fintech"})

        self.assertEqual(response.status_code, 403)
        self.assertIn("paid plans only", str(response.data["detail"]).lower())

    def test_corper_on_free_trial_cannot_search_companies_by_sector_and_state(self):
        self.client.force_authenticate(self.corper_user)

        response = self.client.get("/api/search/companies/", {"sector": "Fintech", "state": "Lagos"})

        self.assertEqual(response.status_code, 403)
        self.assertIn("paid plans only", str(response.data["detail"]).lower())

    def test_corper_on_paid_plan_can_search_companies_by_sector_and_state(self):
    ...
    def test_public_sector_stats_counts_companies_by_state_and_city(self):
        second_company_user = User.objects.create_user(
            email="sector-extra@company.ng",
            password="CompanyPass123!",
            role="company",
            email_verified=True,
        )
        second_company = ensure_company_profile(second_company_user)
        second_company.company_name = "Fintech Field Office"
        second_company.company_address = "20 Market Street"
        second_company.company_location_state = "Ogun, Abuja FCT"
        second_company.company_location_city = "Abeokuta"
        second_company.company_sector = "Fintech"
        second_company.desired_corper_description = "Payments support placement."
        second_company.desired_qualification = "B.Sc"
        second_company.desired_age_range = "21-29"
        second_company.desired_field_of_study = "Accounting"
        second_company.desired_skills = "Excel"
        second_company.desired_experience = "Entry level"
        second_company.verification_status = second_company.VerificationStatus.VERIFIED
        second_company.approval_status = second_company.ApprovalStatus.APPROVED
        second_company.save()

        response = self.client.get("/api/search/company-sector-stats/", {"sector": "Fintech"})

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["total_companies"], 2)
        states = {state["state"]: state for state in response.data["states"]}
        self.assertEqual(len(states), 37)
        self.assertEqual(states["Lagos"]["count"], 1)
        self.assertEqual(states["Lagos"]["cities"], [{"city": "Lekki", "count": 1}])
        self.assertEqual(states["Ogun"]["count"], 1)
        self.assertEqual(states["Ogun"]["cities"], [{"city": "Abeokuta", "count": 1}])
        self.assertEqual(states["Federal Capital Territory"]["count"], 1)
        self.assertEqual(
            states["Federal Capital Territory"]["cities"],
            [{"city": "Abeokuta", "count": 1}],
        )
        self.assertEqual(states["Abia"]["count"], 0)
        self.assertEqual(states["Abia"]["cities"], [])

    def test_sector_stats_do_not_require_active_corper_subscription(self):
