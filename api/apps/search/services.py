...
def filter_companies_for_directory(*, queryset: Any, params: Any) -> Any:
    ...
    for field_name, value in text_filters.items():
        cleaned = (value or "").strip()
        if cleaned:
            if field_name == "company_location_state":
                queryset = queryset.filter(_company_state_query(cleaned))
            else:
                queryset = queryset.filter(**{f"{field_name}__icontains": cleaned})
...
def _company_state_query(value: str) -> Q:
    normalized = _normalized(value)
    aliases = STATE_FILTER_ALIASES.get(normalized, (value,))
    query = Q()
    for alias in aliases:
        query |= Q(company_location_state__icontains=alias)
    return query
