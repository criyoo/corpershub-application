...
local-api-check:
	@cd api && python3 manage.py check --settings=config.settings.local

local-api-test:
	@cd api && python3 manage.py test --settings=config.settings.test

local-migrate:
	@cd api && python3 manage.py migrate

local-seed:
	@cd api && python3 manage.py seed_demo_data
