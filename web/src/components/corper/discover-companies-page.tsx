...
import { getNigerianStateBySlug } from "@/lib/nigerian-reference-data";
...
type SectorCompanyFilter = {
  name: string;
  backHref?: string;
  backLabel?: string;
};

type StateCompanyFilter = {
  name: string;
};
...
export function DiscoverCompaniesPage({
  ...
  sectorFilter?: SectorCompanyFilter;
  stateFilter?: StateCompanyFilter;
}) {
  ...
  const stateNameFromQuery = getNigerianStateBySlug(searchParams.get("state") ?? "");
  const activeStateFilter = stateFilter ?? (stateNameFromQuery ? { name: stateNameFromQuery } : undefined);
  const sectorCompanyParams = new URLSearchParams();
  if (sectorFilter) {
    sectorCompanyParams.set("sector", sectorFilter.name);
  }
  if (activeStateFilter) {
    sectorCompanyParams.set("state", activeStateFilter.name);
  }
  const companiesPath = sectorFilter
    ? `/search/companies/?${sectorCompanyParams.toString()}`
  ...
  const companyTitle = sectorFilter
    ? activeStateFilter
      ? `${sectorFilter.name} Companies in ${activeStateFilter.name}`
      : `${sectorFilter.name} Companies`
    : "Browse Companies";
  const companyCount = companies.data?.count ?? 0;
  const companyCountNoun = companyCount === 1 ? "company" : "companies";
  ...
  return (
    <main className="min-h-screen">
      <div className="mx-auto min-h-screen max-w-[1680px] px-4 py-2 md:px-8 md:py-3 lg:px-10">
        {!publicBrowse && sectorFilter ? (
          <BackButton
            fallbackHref={sectorFilter.backHref ?? "/sectors"}
            label={sectorFilter.backLabel ?? "Back to sector"}
          />
        ) : !publicBrowse ? (
          <Link
        ...
        <section className={`${publicBrowse ? "mt-8" : "mt-10"} rounded-[34px] border border-white/10 bg-white/[0.07] px-4 py-3 shadow-glow backdrop-blur-xl md:px-7 lg:px-8`}>
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <p className="pt-1 text-xs uppercase tracking-[0.26em] text-lime">
              {sectorFilter ? "Sector companies" : "Discover companies"}
            </p>
            {!sectorFilter ? visibleNowCard : null}
          </div>

          <div className="mx-auto mt-5 max-w-3xl text-center">
            <h1 className="font-display text-4xl text-white md:text-3xl">
              {companyTitle}
            </h1>
          </div>
          <div className="mx-auto mt-3 max-w-3xl text-center">
            <p className="max-w-3xl text-base">
              {sectorFilter
                ? activeStateFilter
                  ? `Showing ${companyCount} registered ${companyCountNoun} in ${activeStateFilter.name} under this sector.`
                  : `Showing ${companyCount} registered ${companyCountNoun} in this sector.`
                : searchQuery
        ...
        {!companies.error && !companies.loading && companies.data?.results.length === 0 ? (
          <Card className="mt-6 text-center">
            ...
            <p className="mt-3 text-sm text-mist">
              {sectorFilter
                ? activeStateFilter
                  ? "There are no registered companies in this state under this sector yet."
                  : "There are no registered companies in this sector yet."
